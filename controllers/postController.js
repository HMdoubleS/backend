...
exports.addPost = (req, res, next) => {
  ...
  if (req.body.file) { // if there is a media upload
    ...
    pool.query(
      `INSERT INTO posts(title, author, posttext, image, creationdate) 
    VALUES ($1, $2, $3, $4, NOW()::timestamp)`, // don't know why creationdate/timestamp is here, that should be auto generated by DB
      [post.title, post.author, post.postText, post.image], // use the POST object you've created
      error => {
        if (error) {
          res.status(400).json({
            error: error
    ...
    pool.query(
      `INSERT INTO posts(title, author, posttext, creationdate)
      VALUES ($1, $2, $3, NOW()::timestamp)`, // again don't know why
      [post.title, post.author, post.postText], // again use the POST object you created
      error => {
        if (error) {
          res.status(400).json({
            error: error
